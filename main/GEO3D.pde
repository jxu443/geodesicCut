...
// ===== rotate 
//pt R(pt Q, float a, pt P, vec N) {float dx=Q.x-P.x, dy=Q.y-P.y, c=cos(a), s=sin(a); return P(P.x+c*dx+s*dy, P.y-s*dx+c*dy); }; // Rotated Q by a around P
//pt R(pt Q, float a) {float dx=Q.x, dy=Q.y, c=cos(a), s=sin(a); return new pt(c*dx+s*dy, -s*dx+c*dy); };                 // Rotated Q by a around origin
//vec R(vec V, vec N) {return new vec(-V.y,V.x);};                                            // V turned right 90 degrees (around N)
...
//float r(pt A, pt B, pt C) {return radius(A,B,C); };                                  // radius of circumcircle
