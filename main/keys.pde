boolean showHelpText=true;
  void showHelp() {
    fill(yellow,50); rect(0,0,height,height); pushMatrix(); translate(20,20); fill(0);
        ...
        text("CORNER: *.:select, p:previous, n:next, o:opposite, s:swing, l:left, r:right, t:turn",0,0); translate(0,20);
        ...
        text("If running Processing (not in browser): A:archiveModel, X:snapPicture , w:writeCorner",0,0); translate(0,20);
  ...
  if (key=='.') {
    C.setMark(); 
    M.hitTriangle();/* C.F.setToPoint(mark); C.pullE(); C.pose(); */ 
    M.writeCorner();
    
    resetForDiskRotation();
  };
  ...
  if (key=='k') {showDiskRotation = !showDiskRotation; System.out.println("showDiskRotation is " + showDiskRotation);} 
...
//  if (key=='z') {};   // used in updateView
  ...
  if (key=='G') {
    resetForDiskRotation();
    println("loading fn["+fni+"]: "+fn[fni]); M.loadMesh();  M.init(); initView(M); M.step=0; println("Loaded a model");};  
  if (key=='H') {M.excludeInvisibleTriangles();  M.compactVO(); M.compactV();  println("healed mesh by removing deleted triangles and compacting"); }; 
