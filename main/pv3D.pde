...
//   vec scaleBy(float s) {x *= s; y *= s; z *= s; return this;}
...
//vec S(vec U, float s, vec V) {return V(U.x+s*V.x,U.y+s*V.y,U.z+s*V.z);};                               // U+sV sum
...
//vec V(pt P, pt Q) {return new vec(Q.x-P.x,Q.y-P.y,Q.z-P.z);};                                          // PQ = Q - P
//vec U(vec V) {float n = V.norm(); if (n<0.0000001) return V(0,0,0); else return V(1./n,V);};           // V / |V| unit of
...
vec Normal(vec V) {                                                                                    // some vector orthogonal to V
  if(abs(V.z)<=min(abs(V.x),abs(V.y))) return V(-V.y,V.x,0); 
  if(abs(V.x)<=min(abs(V.z),abs(V.y))) return V(0,-V.z,V.y);
  return V(V.z,0,-V.x);
  }
//float mixed(vec U, vec V, vec W) {return dot(U,cross(V,W));}                                          // U*(VxW)  mixed product (signed scalar)
...
//pt P(pt P, flo at s, vec V) {return new pt(P.x+s*V.x,P.y+s*V.y,P.z+s*V.z);}                           // P+sV
...
//void vert(pt P) {vertex(P.x,P.y,P.z);};                                           // vertex for shading or drawing (between BeginShape() ; and endShape();)
